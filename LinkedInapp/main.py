...
load_dotenv()
email = os.environ.get('LIusername')
password = os.environ.get("LIpass")
print(password)
print(email)
...
class objecttocsv():
    ...
    def objecttocsvfunc(self):
        csvfile = self.to_csv(encoding='utf-8')

def writeToBlob():
    blob_block = ContainerClient.from_connection_string(
        conn_str= "DefaultEndpointsProtocol=https;AccountName=linkedinsink;AccountKey=0t0+MonMX/Kwyy71zwl9n96pIBQC8Aa2iUFuCQP6gdKizhVSRRMOI1XVHF8I4rhhGdtCkSkWAoFc1itXerWEEg==;EndpointSuffix=core.windows.net",
        container_name= "job-posts"
        )
    output = io.StringIO()
    name = f"{object}.csv"
